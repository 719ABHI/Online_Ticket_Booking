...
const dbURI = "mongodb://mongodb:27017/trainbooking"; // Use for Docker setup
mongoose.connect(dbURI, { useNewUrlParser: true, useUnifiedTopology: true })
    .then(() => console.log('MongoDB connected'))
    .catch((err) => console.log('MongoDB connection error:', err));
...
app.post('/api/updateBookedSeats', async (req, res) => {
    ...
    try {
        ...
        const availableSeats = train.seatsAvailable - train.bookedSeats.length;
        if (seats.length > availableSeats) {
            return res.status(400).json({ message: 'Not enough seats available' });
        }
        ...
        const bookingDetails = {
            ...
            from: updatedTrain.source,
            to: updatedTrain.destination,
            date: updatedTrain.departureDate,
            seats,
            amount
        };
